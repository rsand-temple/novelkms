...
A `SubscriptionAccessFilter` was added.
...
```text
novelkms-frontend/src/components/settings/BillingPanel.jsx
```
...
```text
novelkms-frontend/src/components/billing/BillingReturnPage.jsx
```
...
* Admin support now has user search, billing inspection, audit display, and grant-family-access. Next admin billing additions: extend trial, revoke/remove family access after restoration semantics are defined, and deeper webhook diagnostics.
