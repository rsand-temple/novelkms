1. # NovelKMS AI Design
   ...
   - OpenAI and Anthropic (Claude) providers both implemented behind the shared `AiProvider` interface.
   - Provider abstraction exists so additional providers can be added without touching callers.
   - Per-user AI credentials are encrypted at rest and can span multiple providers and labels.
   ...
   The backend is provider-agnostic outside provider implementation classes. `AiReviewService` holds a `Map<String, AiProvider>` keyed by provider string and resolves the right instance from the stored credential at call time.

   Current interface (all methods synchronous):
   ...
   - `OpenAiProvider` — key `OPENAI`, default model `gpt-5.4`. Uses `Authorization: Bearer` auth and `response_format: json_object` for the review JSON contract.
   - `AnthropicProvider` — key `ANTHROPIC`, default model `claude-sonnet-4-6`. Uses `x-api-key` + `anthropic-version: 2023-06-01` headers. System prompt goes as a top-level `"system"` field (not a message role). `max_tokens` is required; values are 4096 for review JSON and 2048/1024 for free-text calls. No `response_format` equivalent — the functional block demands pure JSON and Claude follows it reliably; `stripCodeFences` handles edge cases.
   ...
   1. Implement `AiProvider` in a new class in the `ai` package.
   2. Add it to the `Map.of(...)` in `NovelKmsServer` alongside the existing entries.
   3. Add the provider key and friendly name to the `PROVIDERS` array in `AiCredentialsPanel.jsx`.
   4. No schema migration, no new endpoints, no DAO changes required.
   ...
   Every meaningful behavior change in prompt output should bump the prompt version. Prompt version constants are shared across providers — the version tracks the prompt *content*, not the transport format.
   ...
   | `chapter-review-v7`    | Current version (prompt content updated; JSON output contract unchanged). |
   | `memory-v2`            | Added one-time author guidance fence to user message.        |
   | `chapter-summary-v1`   | One-paragraph chapter summary (free text).                   |
   | `chapter-summary-v2`   | Added one-time author guidance fence to user message.        |
   | `book-summary-v1`      | Whole-book synopsis built from chapter summaries, ≤1000 words. |
   | `book-summary-v2`      | Added one-time author guidance fence to user message.        |
   ...
   - **Functional** — the JSON output contract NovelKMS consumes (field requirements, severity/codex enums, anchorText spec, response shape, empty-array fallback). Scope-aware via `%unit%`/`%categories%` substitution. Non-editable at any scope; lives as `functionalBlock()` in each provider implementation. Both `OpenAiProvider` and `AnthropicProvider` carry identical copies.
   ...
   - **Chapter summary** — one human-readable paragraph per chapter, generated from the chapter prose (`chapter-summary-v2`), optionally hand-edited (`EDITED`). One per chapter, overwrite on regenerate. Created/edited/cleared from the chapter nav context menu.
   - **Book summary** — one synopsis per book, ≤ ~1000 words (`book-summary-v2`), generated **entirely from the chapter summaries** in book order, never the manuscript prose (a full book is too large to summarize reliably in one bite). One per book, overwrite on regenerate, optionally hand-edited.
