...
export default function ImportDialog({ open, onClose, projectId, onSuccess }) {
  ...
  const [result, setResult]       = useState(null)  // ImportResult on success
  ...
  const handleFileChange = (e) => {
    ...
    setResult(null)
    setErrorMsg(null)
    // Pre-fill book title from filename if user hasn't typed one
    if (selected && !bookTitle.trim()) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <TextField
          label="Book Title"
          helperText="Leave blank to use the document title or filename."
          value={bookTitle}
        ...
        {!result && !errorMsg && (
          <Typography variant="caption" color="text.secondary">
            The importer detects Heading 1 and Heading 2 styles to build the
            book hierarchy. If your manuscript uses Heading&nbsp;1 for parts and
            Heading&nbsp;2 for chapters, both will be imported. If only
            Heading&nbsp;1 (or Heading&nbsp;2) is present, those become
            chapters. Scene breaks (*, #, ---) split scenes within chapters.
          </Typography>
        )}
        ...
        {result && (
          <Box sx={{ mt: 2 }}>
            ...
            <Box
              component="ul"
              sx={{ mt: 0.5, mb: 1, pl: 2, '& li': { typography: 'body2' } }}
            >
              {result.partCount > 0 && <li>{result.partCount} part{result.partCount !== 1 ? 's' : ''}</li>}
