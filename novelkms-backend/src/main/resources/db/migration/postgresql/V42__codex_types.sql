...
SELECT gen_random_uuid(), c.id, 'role', 'Role / Archetype', 'SELECT', '["Protagonist","Antagonist","Deuteragonist","Foil","Mentor","Ally","Love interest","Supporting","Minor"]', 'The structural role this character plays in the story.', TRUE, 0
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'age', 'Age', 'SHORT_TEXT', NULL, 'Age or apparent age.', TRUE, 1
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'summary', 'One-line summary', 'SHORT_TEXT', NULL, 'A single sentence capturing who this character is.', TRUE, 2
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'want', 'Want (external goal)', 'LONG_TEXT', NULL, 'What the character is consciously pursuing.', TRUE, 3
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'need', 'Need (internal)', 'LONG_TEXT', NULL, 'What the character actually needs, often unknown to them.', TRUE, 4
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'conflict', 'Central conflict / flaw', 'LONG_TEXT', NULL, 'The core flaw or tension driving the character.', TRUE, 5
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'arc', 'Arc', 'LONG_TEXT', NULL, 'How the character changes across the story.', TRUE, 6
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'appearance', 'Appearance', 'LONG_TEXT', NULL, 'Distinctive physical details worth keeping consistent.', TRUE, 7
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'voice', 'Voice / speech notes', 'LONG_TEXT', NULL, 'How this character speaks and sounds on the page.', TRUE, 8
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'relationships', 'Relationships', 'LONG_TEXT', NULL, 'Key relationships and how they stand.', TRUE, 9
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'secrets', 'Secrets', 'LONG_TEXT', NULL, 'What the character hides, and from whom.', TRUE, 10
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER'
UNION ALL
SELECT gen_random_uuid(), c.id, 'authorNotes', 'Author notes (private)', 'LONG_TEXT', NULL, 'Private notes for you; never shared with the AI.', FALSE, 11
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'CHARACTER';

-- VOICE: 10 fields (order = array order from V33)
INSERT INTO codex_type_field
    (id, chapter_id, field_key, label, input_type, options, help, feeds_ai, display_order)
SELECT gen_random_uuid(), c.id, 'appliesTo', 'Applies to (character / narrator)', 'SHORT_TEXT', NULL, 'Whose voice this sheet describes.', TRUE, 0
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'register', 'Register', 'SELECT', '["Formal","Neutral","Casual","Vulgar","Archaic","Mixed"]', 'The overall formality of this voice.', TRUE, 1
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'diction', 'Diction & vocabulary', 'LONG_TEXT', NULL, 'Characteristic word choices and vocabulary level.', TRUE, 2
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'rhythm', 'Sentence rhythm & length', 'LONG_TEXT', NULL, 'Typical sentence length and cadence.', TRUE, 3
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'tics', 'Verbal tics / catchphrases', 'LONG_TEXT', NULL, 'Repeated phrases, fillers, or habits of speech.', TRUE, 4
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'dialect', 'Dialect / accent', 'LONG_TEXT', NULL, 'Regional or class markers rendered on the page.', TRUE, 5
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'dos', 'Does', 'LONG_TEXT', NULL, 'Things this voice characteristically does.', TRUE, 6
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'donts', 'Never does', 'LONG_TEXT', NULL, 'Things this voice never does.', TRUE, 7
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'samples', 'Sample lines', 'LONG_TEXT', NULL, 'A few representative lines in this voice.', TRUE, 8
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE'
UNION ALL
SELECT gen_random_uuid(), c.id, 'authorNotes', 'Author notes (private)', 'LONG_TEXT', NULL, 'Private notes for you; never shared with the AI.', FALSE, 9
    FROM chapter c WHERE c.codex_id IS NOT NULL AND c.codex_category = 'VOICE';
