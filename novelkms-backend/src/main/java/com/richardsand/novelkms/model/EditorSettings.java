 ...
 *
 * <p>{@code scope} is {@code SYSTEM | USER | PROJECT}. {@code projectId} is set
 * for PROJECT rows; it is null for SYSTEM and USER rows.
 *
 * <p>When returned from a resolve endpoint, inspect {@code scope} to know which
 * level the effective definition came from — i.e. whether the project is
 * overriding ({@code PROJECT}) or inheriting the user default ({@code USER}) or
 * the factory default ({@code SYSTEM}). Mirrors the {@link Style} model so the
 * frontend override-badge logic is identical.
 *
