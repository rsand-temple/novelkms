...
public class OAuthService implements AutoCloseable {
    ...
    public CallbackResult callback(String providerName, String code, String state) throws Exception {
        ...
        if (!provider.equals(oauthState.provider())) throw new IllegalArgumentException("OAuth provider does not match state");
        ...
        if (profile.email() == null || profile.email().isBlank()) throw new IllegalArgumentException("OAuth provider did not return an email address");
        if (!profile.emailVerified()) throw new IllegalArgumentException("OAuth provider did not verify the email address");
    ...
    private OAuthProfile exchange(String providerName, String code) throws Exception {
        ...
        if (!providerName.equalsIgnoreCase("meta")) get.setHeader("Authorization", "Bearer " + accessToken);
    ...
    private JSONObject executeJson(org.apache.http.client.methods.HttpUriRequest request) throws IOException {
        try (CloseableHttpResponse response = http.execute(request)) {
            ...
            if (response.getStatusLine().getStatusCode() / 100 != 2) throw new IOException("OAuth endpoint returned " + response.getStatusLine().getStatusCode());
    ...
    private NovelKmsConfig.OAuthProvider provider(String name) {
        ...
        if (p == null || p.clientId == null || p.clientSecret == null) throw new IllegalArgumentException("OAuth provider is not configured: " + name);
        return p;
    }
    ...
    private static String safeReturnPath(String value) { return value != null && value.startsWith("/") && !value.startsWith("//") ? value : "/"; }
