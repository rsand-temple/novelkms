...
public class SessionService {
    public record AuthenticatedSession(AppUser user, Set<String> roles) {
    }

    private final AuthDao             dao;
    private final NovelKmsConfig.Auth config;

    public SessionService(AuthDao dao, NovelKmsConfig.Auth config) {
        this.dao = dao;
        this.config = config;
    }
    ...
    public Optional<AuthenticatedSession> authenticate(String raw) throws Exception {
        ...
        if (result.isPresent()) {
            dao.touchSession(hash);
            AppUser     user  = result.get().user();
            Set<String> roles = dao.findRolesForUser(user.id());
            return Optional.of(new AuthenticatedSession(user, roles));
        }

        return Optional.empty();
    }

    public void revoke(String raw) throws Exception {
        if (raw != null) {
            dao.revokeSession(CryptoTokens.sha256(raw));
        }
    }
}